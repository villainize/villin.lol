(() => {
  ...
  function sparkWorkspace(token) {
    ...
    let slots=Array.isArray(saved?.slots)?saved.slots.slice(0,limit):[]; while(slots.length<limit) slots.push(null);
    ...
    async function runSimulation(){const chosen=slots.filter(Boolean);if(chosen.length<2){output.replaceChildren(el('p','Add at least two units before running the simulator.','result-note'));return;}run.disabled=true;run.textContent='Simulating…';output.replaceChildren(el('p','Project Sparkle is checking orders and positions…','loading-note'));try{await loadDetails();const entries=slots.map((slot,index)=>{if(!slot)return{id:'E',position:positions[index]};const unit=lookup.get(slot.key);return{id:unit.id,type:slot.type||'sbb',bbOrder:+slot.bbOrder||index+1,position:slot.position||positions[index]};});const sim=new window.SparkSimulator({getUnit:id=>{const entry=entries.find(item=>String(item.id)===String(id));const unit=entry&&lookup.get(slots[entries.indexOf(entry)]?.key);return unit?detailCache.get(unit.key):null;}});const results=await sim.run(entries,{sortResults:true,threshold:0,maxResults:5});output.replaceChildren(el('p',`${results.length} viable sequence${results.length===1?'':'s'} found.`,'result-note'));results.slice(0,5).forEach((result,index)=>{const card=el('article',undefined,'spark-result');card.append(el('span',index===0?'BEST ORDER':`OPTION ${index+1}`,'role-chip'),el('h4',`${(result.weightedPercentage*100).toFixed(1)}% weighted spark`));const stats=el('div',undefined,'spark-result-stats');[['Actual sparks',result.actualSparks],['Possible sparks',result.possibleSparks]].forEach(([label,value])=>{const stat=el('div',undefined,'spark-result-stat');stat.append(el('small',label),el('strong',String(value)));stats.append(stat);});card.append(stats);const order=el('div',undefined,'spark-result-order');result.squad.filter(item=>item.id!=='E').sort((a,b)=>a.bbOrder-b.bbOrder).forEach(item=>{const unit=units.find(u=>String(u.id)===String(item.id)&&u.server===server.value)||units.find(u=>String(u.id)===String(item.id));order.append(el('span',`${item.bbOrder}. ${unit?.name||item.id} · ${item.position}`));});card.append(order);output.append(card);});}catch(error){output.replaceChildren(el('p',error.message||'The simulator could not process this squad.','result-note'));}finally{run.disabled=false;run.textContent='Run spark simulation';}}
